...
{
  // Higher subdivisions so the per-pixel sun disc renders as a clean
  // circle. At 48x24 the sun was rendering as a triangle artifact.
  const skyGeo = new THREE.SphereGeometry(900, 96, 48);
...
// Ambient bumped from 0.35 -> 0.55 so cars never lose their body color
// to the dark sky. Combined with the lower envMapIntensity, body colors
// now read at any angle.
const ambient = new THREE.AmbientLight(0x9bb6ff, 0.55);
scene.add(ambient);
...
// Hemi bumped 1.2 -> 1.5 — reads as soft daylight bounce on horizontal
// surfaces (top of cars + road) and underbelly fill from the ground.
const hemi = new THREE.HemisphereLight(0xa8c8ff, 0x4a3060, 1.5);
scene.add(hemi);
...
let finishOrbitUntil = 0;     // perf.now() timestamp until which post-finish camera orbit holds
let finishOrbitT = 0;         // accumulating phase for the orbit camera
...
function updateCamera(dt) {
  ...
  // Camera roll — leans WITH lateral velocity. When player drifts right,
  // camera rolls slightly right so the horizon tilts. AAA-style lean.
  if (car && Number.isFinite(car.lateralV)) {
    const targetRoll = -Math.max(-1, Math.min(1, car.lateralV / 22)) * 0.10;
    const cur = camera.rotation.z || 0;
    camera.rotation.z = cur + (targetRoll - cur) * Math.min(1, dt * 5);
  }
...
function recordSector(idx, time) {
  ...
  if (d && prev != null) {
    ...
    // Floating popup celebrates the delta — green for faster, hot for slower.
    if (delta < -0.05) {
      showScorePopup(`S${idx} ${delta.toFixed(2)}`, 50, 22, "green");
    } else if (delta > 0.05) {
      showScorePopup(`S${idx} +${delta.toFixed(2)}`, 50, 22, "hot");
    } else {
      showScorePopup(`S${idx} =${delta.toFixed(2)}`, 50, 22, "cyan");
    }
  }
...
function tick(dt) {
  ...
  // Shift audio cue — rising edge. Upshifts get a small exhaust pop too,
  // since real cars often pop on full-throttle upshifts.
  if (car.shiftEvent) {
  ...
  // Brake squeal — heavy braking at low speed (pad-on-rotor effect when
  // the car is almost stopped). Throttled by the time since last squeal.
  if (i.brake && Math.abs(car.speed) < 20 && Math.abs(car.speed) > 4) {
    if (!car._lastSquealAt || performance.now() - car._lastSquealAt > 700) {
      playBrakeSqueal();
      car._lastSquealAt = performance.now();
    }
  }
  // First draft tip.
  ...
  // Tail-light halo glow — tiny sphere bloom behind each lamp.
  const tailHaloMats = car.group?.userData?.tailHaloMats;
  if (tailHaloMats) {
    const targetOp = i.brake ? 0.55 : (i.throttle ? 0.10 : 0.28);
  ...
  if (car._offRoad && Math.abs(car.speed) > 8 && Math.random() < 0.6) {
    const sinH = Math.sin(car.heading);
    const cosH = Math.cos(car.heading);
    const rearOffsetZ = -1.4;
    const rwx = car.group.position.x + sinH * rearOffsetZ;
    const rwz = car.group.position.z + cosH * rearOffsetZ;
    const rightX = cosH, rightZ = -sinH;
    for (const side of [-1, 1]) {
      if (Math.random() < 0.7) {
        const tx = rwx + rightX * side * 0.85;
        const tz = rwz + rightZ * side * 0.85;
        spawnSmoke(tx, 0.3, tz);
      }
    }
  }
...
function computeStandings() {
  ...
  const playerSpeed = Math.max(8, Math.abs(car.speed));
  const entries = [{ name: "You", total: playerTotal, isPlayer: true, speed: playerSpeed }];
  for (const r of rivals) {
    entries.push({ name: r.name, total: r.laps * trackLen + r.s, isPlayer: false, speed: Math.max(8, Math.abs(r.speed || 30)) });
  }
  entries.sort((a, b) => b.total - a.total);
  // Gap to leader, in seconds, using each car's own current speed as the
  // approximation rate. Leader has gap=0; others positive.
  const leader = entries[0];
  for (const e of entries) {
    const distance = leader.total - e.total;
    e.gap = distance / e.speed;   // simple: distance behind / your own speed
  }
...
let titleCinemaT = 0;
// Post-finish orbit — slow cinematic sweep around the player car for
// 2.4-3.0s before the overlay drops. Camera arcs from low-rear to
// high-side while pulling slightly closer.
function tickFinishOrbit(dt) {
  finishOrbitT += dt;
  const t = finishOrbitT;
  // Sweep from chase position → low-side hero shot.
  const angle = -Math.PI * 0.20 + t * 0.65;     // start 0.2π behind, sweep right
  const dist = 8.5 - Math.min(2.5, t * 1.0);    // pull in slowly
  const height = 1.6 + Math.sin(t * 0.9) * 0.6;
  const cx = car.group.position.x + Math.sin(car.heading + angle) * dist;
  const cz = car.group.position.z + Math.cos(car.heading + angle) * dist;
  camera.position.set(cx, car.group.position.y + height, cz);
  camera.lookAt(car.group.position.x, car.group.position.y + 0.9, car.group.position.z);
}
...
function loop(now) {
  ...
  if (!introActive && !spectatorMode && !photoMode && !replayPlaying) {
    if (finishOrbitUntil > performance.now() && car) {
      // Post-finish cinematic orbit — slow camera sweep around the player.
      tickFinishOrbit(dt);
    } else if (finishOrbitUntil > 0 && finishOrbitUntil <= performance.now()) {
      // Orbit just finished — drop the overlay in.
      finishOrbitUntil = 0;
      const fov = document.getElementById("finish-overlay");
      if (fov) fov.hidden = false;
    } else if (!overlay.hidden && !running) {
  ...
  if (projected.s < lastTrackS - track.length * 0.5 && running) {
    ...
    resetSectors();
    // Flash the lap counter pill so the player notices the lap rolled.
    const lapEl = document.getElementById("lap")?.parentElement;
    if (lapEl) {
      lapEl.classList.remove("is-bumping");
      void lapEl.offsetWidth;
      lapEl.classList.add("is-bumping");
      setTimeout(() => lapEl.classList.remove("is-bumping"), 400);
    }
  }
  ...
  if (running && lap > lapsTotal() && !finishShown) {
    finishShown = true;
    running = false;
    // Before the overlay drops, hold the camera in an orbital sweep for
    // 2.4s. Race winners get 3.0s. Pure cinematic moment.
    finishOrbitUntil = performance.now() + (standings.place === 1 ? 3000 : 2400);
    finishOrbitT = 0;
...
function renderStandings(top, playerPlace) {
  ...
  top.forEach((e, i) => {
    ...
    // Gap to leader — leader shows lap leader pill, others show "+1.2"
    const gapStr = i === 0
      ? `<span class="gap leader">P1</span>`
      : `<span class="gap">+${Math.min(99.9, e.gap || 0).toFixed(1)}</span>`;
    html += `<li${cls}><span class="rank">${place}</span>${dot}<span class="name">${e.name}</span>${gapStr}</li>`;
  });
  ol.innerHTML = html;
}
...
function showFinish(standings) {
  ...
  // Defer overlay reveal until the orbit cinematic finishes — the loop
  // will flip overlay.hidden = false once finishOrbitUntil expires.
  overlay.hidden = true;
...
function startRace() {
  ...
  // Controls reminder strip — fades in for 3.5s at race start. Players
  // who already know the controls can ignore; first-timers see them.
  const cstrip = document.getElementById("controls-strip");
  if (cstrip) {
    cstrip.classList.remove("is-show");
    void cstrip.offsetWidth;
    cstrip.classList.add("is-show");
    setTimeout(() => cstrip.classList.remove("is-show"), 3600);
  }
...
// Next track — pick a different random track and restart.
...
// Back to menu — reopen the title overlay (pre-race screen).
